    ...
    while counter < maxGames:
        ...
        while not done:
            ...
            if (fcounter%FPS/8 == 0):
                ...
                newAction = QLearning.epsilon_greedy(QL,0.1,newState)
                if newAction == 'jump':
                    bird.msec_to_climb = Bird.CLIMB_DURATION
        ...
        for i in range(len(episode)-2):
            if episode[i+1][0][1] >= 0 and episode[i+1][0][1] <= 3:
                QL.update(episode[i][0],episode[i][1],reward_pass,episode[i+1][0])
            else:
                QL.update(episode[i][0],episode[i][1],reward,episode[i+1][0])
        QL.update(episode[len(episode)-2][0],episode[len(episode)-2][1],reward_die,episode[len(episode)-1][0])
